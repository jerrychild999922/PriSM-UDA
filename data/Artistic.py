...
class StyleDataSet(Dataset):
    def __init__(self, root, is_train, transform=None):
        ...
        self.root_parsing = os.path.join(root, 'parsing')
    ...
    def get_image(self):
        ...
        for temp_info in info_list:
            temp_name = os.path.join(self.image_root, temp_info)
            parsing_name = os.path.join(self.root_parsing, temp_info.split('.')[0] + ".png") 

            #Points = np.load(os.path.join(self.root_landmark, temp_info[:-3] + "npy"))
            Points = np.load(os.path.join(self.root_landmark, temp_info + ".npy"))
            #print(os.path.join(self.root_landmark, temp_info + ".npy"))
            #input()
